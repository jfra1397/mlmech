...
model_type = "vgg" # "vgg", mobilenetv2", "resnet", "simple_decoder", "unet", "advanced_decoder", "segnet"
...
if model_type == "vgg":
    ...
    layered_view(model, legend=True, scale_xy = 1, font=font, to_file="plots/models/vgg16.png").show()
    ...
    model = Model(inputs=encoder.inputs, outputs=encoder.output)
    font = ImageFont.truetype("arial.ttf", 24*2)  # using comic sans is strictly prohibited!
    layered_view(model, legend=True, font=font, scale_xy=3, to_file="plots/models/mobilenet_v2.png", scale_z=0.0001, max_z=0.1).show()
    ...
    model = Model(inputs=encoder.inputs, outputs=encoder.output)
    font = ImageFont.truetype("arial.ttf", 24*2)  # using comic sans is strictly prohibited
    layered_view(model, legend=True, font=font, scale_xy=3, to_file="plots/models/resnet.png", scale_z=0.0001, max_z=0.1).show()
    ...
    layered_view(model, legend=True, font=font, scale_xy = 0.1, to_file="plots/models/simple_decoder.png", scale_z = 10, index_ignore=[0]).show()
elif model_type == "unet":
    model = tf.keras.models.load_model("results/julian/unet_4/model.tf")
    font = ImageFont.truetype("arial.ttf", 24)  # using comic sans is strictly prohibited!
    layered_view(model, legend=True, scale_xy = 1, font=font, to_file="plots/models/unet.png").show()
